...
const ModificarPelicula = ({ pelicula = null, onSuccess, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                titulo: pelicula?.titulo || titulo.trim(), 
                nuevoTitulo: titulo.trim(),
                director: director.trim(),
                duracion: dur,
                genero,
            };
            const resp = await authApi.patch('/v1/peliculas/modificar', payload);
            ...
            if (status === 403) {
                setError('No tienes permisos para modificar esta película o la operación está prohibida.');
            } else {
    ...
    // when editing, populate fields from `pelicula` prop
    useEffect(() => {
        if (pelicula) {
            setTitulo(pelicula.titulo || '');
            setDirector(pelicula.director || '');
            setDuracion(pelicula.duracion ? String(pelicula.duracion) : '');
            setGenero(pelicula.genero?._id ? String(pelicula.genero._id) : (pelicula.genero ? String(pelicula.genero) : ''));
        }
    }, [pelicula]);
    ...
    return (
        <div className="container auth-container">
            <header>
                <h1><img src="/movie-logo.svg" className="logo-img" alt="logo" /> Modificar Película</h1>
            ...
            <form onSubmit={handleSubmit}>
                <div className="form-group">
                    ...
                    <button type="submit" className="btn btn-primary" disabled={loading} style={{ marginTop: '12px' }}>
                        {loading ? 'Guardando...' : 'Modificar Película'}
                    </button>
                </div>
            </form>
        </div>
